...
import {
  ...
  GetCuisineValidationSchema,
} from './CuisineValidator';

import { BadRequestException } from '../../shared/exceptions/http.exceptions';

export const getCuisineValidator = asyncHandler(async (
  req: Request,
  _: Response,
  next: NextFunction
) => {
  ...
  const input = new GetCuisineValidationSchema();
  input.admin_user_id = String(req.body.admin_user_id);



  await validateCuisine(input);

  next();
});

async function validateCuisine(credentials: BaseValidationSchema): Promise<void> {
